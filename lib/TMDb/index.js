...
/* Get started with the basics of the TMDB API at: https://developer.themoviedb.org/docs/getting-started */
...
export const getPopularMovies = async (lang, pages) => { /* Set default values if parameters are undefined */
  if (!pages)
  ...
  try { /* Make a GET request to fetch popular movies from the TMDB API */
  ...
  } catch (error) { /* If an error occurs during the request, the code inside the catch block is executed */
    console.log(error);
  }
};
...
export const getTopRatedMovies = async (lang, pages) => {
  ...
  try { /* Make a GET request to fetch top-rated movies from the TMDB API */
    const response = await axios.get(
...
export const getNowPlaying = async (lang, pages) => {
  ...
  try { /* Make a GET request to fetch currently playing movies from the TMDB API */
    const response = await axios.get(
...
export const getUpcoming = async (lang, pages) => {
  ...
  try { /* Make a GET request to fetch upcoming movies from the TMDB API */
    const response = await axios.get(
...
export const getMovieDetails = async (id, lang) => {
  ...
  try { /* Make a GET request to fetch detailed information about a specific movie from the TMDB API */
    const response = await axios.get(
...
export const getRecommendations = async (id, page, lang) => {
  if (!lang) lang = "en-US";
  if (!page) page = 1;
  try {
    const response = await axios.get(
      `  https://api.themoviedb.org/3/movie/${id}/recommendations?language=${lang}&page=${page}`,
      header
    );
    return response.data;
  } catch (error) {
